...
function veventLines({ uid, summary, description, startUtc, endUtc, dtstamp, location }) {
  ...
  return [
    ...
    foldIcalLine(`SUMMARY:${escIcalText(summary)}`),
    foldIcalLine(`DESCRIPTION:${escIcalText(description)}`),
    location ? foldIcalLine(`LOCATION:${escIcalText(location)}`) : "",
    "STATUS:CONFIRMED",
    "TRANSP:OPAQUE",
    "END:VEVENT",
  ].filter(Boolean);
...
export function buildEventIcal(event) {
  return [...VCALENDAR_HEAD, ...veventLines(event), "END:VCALENDAR"].join("\r\n") + "\r\n";
}

// One VCALENDAR holding every event of a booking — attached to the client's
// confirmation email so the whole booking lands in their calendar at once.
// METHOD:PUBLISH (not REQUEST) keeps this an informational copy, so mail
// clients file it as "add to calendar" rather than an invite awaiting a reply.
export function buildCalendarIcal(events) {
  return [
    ...VCALENDAR_HEAD,
    "METHOD:PUBLISH",
...
// Creates the event, refusing to overwrite one that already exists at this
// UID — that is what "If-None-Match: *" buys us. With a UID derived from the
// date and slot, the CalDAV server itself becomes the lock on a slot, which
// a read-then-write check cannot be: iCloud does not show a freshly written
// event to a REPORT query for some seconds afterwards.
//
// The thrown error carries .status so callers can tell "slot already taken"
// (412, some servers 409) apart from a genuine failure.
export async function putEvent({ calendarUrl, email, password }, uid, ical) {
  ...
  if (!res.ok && res.status !== 201 && res.status !== 204) {
    const txt = await res.text().catch(() => "");
    const err = new Error(`CalDAV PUT failed: ${res.status} ${res.statusText} — ${txt.slice(0, 300)}`);
...
// True when a PUT was refused because something already occupies that UID.
export function isSlotTakenError(err) {
  return err && (err.status === 412 || err.status === 409);
}
