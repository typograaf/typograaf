...
export type Busy = { date: string; slots: Slot[] }
type View = 'booking' | 'confirm' | 'error'

export default function CalendarBooking({ initialBusy }: { initialBusy: Busy[] | null }) {
  ...
  const [busy, setBusy] = useState<Busy[] | null>(initialBusy)
  ...
  // Load availability ------------------------------------------------------
  // The server component pre-renders with cached busy data, so we only
  // refresh in the background to catch bookings made since the cached
  // HTML was generated. Skip entirely if SSR already gave us data.
  useEffect(() => {
  ...
  }, [earliest, latest, initialBusy])

  // After mount, do one background refresh to catch bookings newer than
  // the SSR cache. Run after a frame so it doesn't compete with hydration.
  useEffect(() => {
    if (!initialBusy) return
    const id = window.setTimeout(() => {
      ...
      fetch(`/api/calendar/availability?from=${from}&to=${to}`)
        ...
        .then((data) => { if (data?.busy) setBusy(data.busy) })
        .catch(() => {})
    }, 1500)
