...
exports.handler = async (event, context) => {
  ...
  console.log('Event source:', event.headers['x-netlify-event']);
  ...
  try {
    ...
    // Process all images without timeout pressure
    const images = await scanDropboxPortfolioComplete(currentToken, portfolioPath);
    ...
    const { error: metaError } = await supabase.from('portfolio_meta').upsert({
      id: 1, // Use a fixed ID for single row
    ...
    console.log(`Synced ${images.length} images from ${projects.length} projects`);
...
async function scanDropboxPortfolioComplete(token, basePath) {
  ...
  try {
    ...
    // Process ALL projects since we're not under time pressure
        ...
        for (const tool of toolFolders) {
          ...
          try {
            ...
            for (const file of imageFiles) {
              ...
              // Calculate aspect ratio from image dimensions if possible
              let aspectRatio = 1.33; // Default
              try {
                aspectRatio = await getImageAspectRatio(imageUrl);
              } catch (err) {
                console.log(`Could not get aspect ratio for ${file.name}, using default`);
              }
...
async function getImageAspectRatio(imageUrl) {
  // This would require image processing, for now return default
  // In a real implementation, you might use a service to get image dimensions
  return 1.33;
}
